...
import carts_icon from '../assets/carts_icon.png';
...
const Navbar = () => {
    ...
    const{ setShowSearch}= useContext(ShopContxt);


  return (
    <div className='flex items-center justify-between py-5 font-medium'>

    <img onClick={()=>setShowSearch(true)}  src={logo} className='w-36' alt="" />
    ...
    <div className='flex items-center gap-6'>
        ...
        <Link to='/cart' className='relative'>
            <img src={carts_icon} className='w-5 min-w-5' alt='' />
    ...
    <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
        <div onClick={()=>setVisible(false)} className='flex flex-col text-gray-600'>
            <div className='flex item-center gap-4 p-3 cursor-pointer'>
                <img className='h-4 ' src={dropdown_icon} alt=""/>
                <p>Back</p>
            </div>
